...
\synopsis{Kill the subprocess specified by the process handle \var{id}}
...
\synopsis{Run \var{cmd} in a separate process}
...
\description
  ...
  buffer at the current point.  It generates a \slang error if the
...
\synopsis{Close the "stdin" of the process \var{id}}
...
\synopsis{Set \var{what} for process \var{pid}}
...
#v-
\description
  \var{pid} is the process handle returned by \ifun{open_process}.  The second
...
#v+
       ...
       "output" :   This parameter determines how output from the process is
                    ...
                    the name or a reference to a S-Lang function that
...
\synopsis{Send a signal to the process \var{pid}}
